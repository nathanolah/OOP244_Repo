...
namespace sdds
{
   ...
   Robot::Robot() : durability(0), arms(0), legs(0) {
      nickname[0] = '\0';
   }
   ...
   void box(Robot& robot1, Robot& robot2) {
      ...
      if (robot1 || robot2) {
         ...
         while (!robot1 && !robot2) {
            if (robot1.getLegs() > robot2.getLegs() || robot1.getArms() > robot2.getArms()) {  // if robot1's speed is greater than robot2's speed 
            ...
            else if (robot2.getLegs() > robot1.getLegs() || robot2.getArms() > robot1.getArms()) { // if robot2's speed is greater than robot1's speed 
               robot1 -= (robot2.getArms().getForce()); // robot2's force value is passed as an argument to minus equals operator overload 
