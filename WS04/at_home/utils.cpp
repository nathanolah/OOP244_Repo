...
namespace sdds {
  ...
  */
   ...
   // Allocates memory for the "dest" string and copies "src" value into "dest".
   // This function contains 3 parameters for the destination string, the source string,
   // and a max length integer used for allocation.
   void copystr(char*& dest, const char* src, unsigned int max_len) {
      ...
      else if (src == nullptr || src[0] == '\0') { // Checks "src" state
         dest = nullptr;
         valid = false; 
      }
      else if (valid) {
         // Checks if src's length is greater than max_len, and allocates memory 
